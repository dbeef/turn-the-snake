...
public class Multishape implements Screen {
public double timerRegulate = 0;
public int tabWysokosci[] = new int[5];
public int tabWysokosci2[] = new int[5];

public int aktualnaWysokosc1;
public int aktualnaWysokosc2;
...
public double actualRed2 = 1;
public double actualGreen2 = 0.996f;
public double actualBlue2 = 0.87f; 
...
public int backgroundColour2 = -1;

public double completeRotation = 0;
...
public Rectangle[] rects4= new Rectangle[400];
	...
	double waysX[] = new double[5];
	double waysY[] = new double[5];
  ...
  double timerSnake2 = 0;
	 ...
	 public class MyActor extends Actor {
	        ...
	        game.sps.setRegion(20, 160, 430, 465);
	        game.sps.setX(165);
	        game.sps.setY(-90);
	        game.sps.setRotation(180);
	        game.sps.setScale(1.4f, 0.318f);
	        ...
	        game.sps2.setRegion(20, 160, 430, 465);
	        game.sps2.setX(165);
	        game.sps2.setY(-305);
	        game.sps2.setRotation(180);
	        game.sps2.setScale(1.4f, 0.318f);
...
public Multishape(final Starter myGdxGame) {
    ...
    aktualnaWysokosc1 = 50;
    aktualnaWysokosc2 = 50;
...
regulatePositions();
	...
	Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);	
		...
		game.batch.draw(game.yellowPentagon,0,200);
		drawSnake();
		drawApple();
	...
	checkFigureCollisions();
		//	System.out.println("apple2 x,y : " + apple2.x + " " + apple2.y);
		//	System.out.println("snakeHead2 x,y : " + snakeHead2.x + " " + snakeHead2.y);
			spawnSnake();
		updateCamera();
		updateCamera2();
...
//	waysY[4] = 0.725;
	...
	for(int a = 0;a<400;a++){
		...
		rects0[a].setSize(2);
		rects0[a].setPosition((float)(waysX[4] + lastX),(float)(waysY[4] + lastY ));
	lastX += waysX[4];
	lastY += waysY[4];
	}
	...
	for(int a = 0;a<400;a++){
		...
		rects4[a] = new Rectangle();
		rects4[a].setSize(2);
		rects4[a].setPosition((float)(waysX[3] + lastX),(float)(waysY[3] + lastY));
	lastX += waysX[3];
	lastY += waysY[3];
	}
}
public void drawFigureRectangles(){
	...
	for(int a =0; a<400;a++){
		game.batch.draw(game.snakeShape,rects4[a].getX(),rects4[a].getY());
	}
}
...
for(int a =0;a<300;a++){
	...
	kolizja = true;}
	if(rects4[a].overlaps(snakeHead) || rects4[a].overlaps(snakeHead2)){lost = true;if(kolizja == false)game.przegrana.play();
	kolizja = true;}
	if(way > 4)way = 0;
	if(way2 > 4)way2 = 0;
...
//Drugi snake
	...
	if(backgroundColour2 != -1){
if(actualRed2 < red[backgroundColour2]){actualRed2 += Gdx.graphics.getDeltaTime()*3;}
if(actualRed2 > red[backgroundColour2]){actualRed2 -= Gdx.graphics.getDeltaTime()*3;}
if(actualGreen2 < green[backgroundColour2]){actualGreen2 += Gdx.graphics.getDeltaTime()*3;}
if(actualGreen2 > green[backgroundColour2]){actualGreen2 -= Gdx.graphics.getDeltaTime()*3;}
if(actualBlue2 < blue[backgroundColour2]){actualBlue2 += Gdx.graphics.getDeltaTime()*3;}
if(actualBlue2 > blue[backgroundColour2]){actualBlue2 -= Gdx.graphics.getDeltaTime()*3;}
	}
	else
	{
		if(actualRed2 < 1){actualRed2 += Gdx.graphics.getDeltaTime()*3;}
		if(actualRed2 > 1){actualRed2 -= Gdx.graphics.getDeltaTime()*3;}
		if(actualGreen2 < 0.996f){actualGreen2 += Gdx.graphics.getDeltaTime()*3;}
		if(actualGreen2 > 0.996f){actualGreen2 -= Gdx.graphics.getDeltaTime()*3;}
		if(actualBlue2 < 0.87f){actualBlue2 += Gdx.graphics.getDeltaTime()*3;}
		if(actualBlue2 > 0.87f){actualBlue2 -= Gdx.graphics.getDeltaTime()*3;}		

		if(actualRed2 > 0.95f && actualRed2 < 1)actualRed2 = 1;
		if(actualGreen2 > 0.91f && actualGreen2 < 0.996f)actualGreen2 = 0.996f;
		if(actualBlue2 > 0.82f && actualBlue2 < 0.87f)actualBlue2 = 0.87f;
		
	}
	
//	Pixmap pixmap = new Pixmap( 480, 400, Pixmap.Format.RGBA8888 );
//    pixmap.setColor((float)actualRed,(float)actualGreen,(float)actualBlue,1);
//    pixmap.fill();
	
//    pixmap2.setColor((float)actualRed2,(float)actualGreen2,(float)actualBlue2,1);
 //   pixmap2.fill();
  ...
  // texSnake1.dispose();
   ...
   ///texSnake1 =  	new Texture(new PixmapTextureData(pixmap, pixmap.getFormat(), false, false, true));
   
   //texSnake2 =  	new Texture(new PixmapTextureData(pixmap2, pixmap.getFormat(), false, false, true));
...
public void goOut(){
	...
	if(tryAgainBoolean == true && timerOut == 0.9f){	game.setScreen(new Multishape(game));
	}
...
public void regulatePositions(){

	timerRegulate += Gdx.graphics.getDeltaTime();
	if(timerRegulate > 0.001f){
	
	if(aktualnaWysokosc1 > tabWysokosci2[way]){aktualnaWysokosc1 -= 1;}
	if(aktualnaWysokosc1 < tabWysokosci2[way]){aktualnaWysokosc1 += 1;}
	
	if(aktualnaWysokosc2 > tabWysokosci[way2]){aktualnaWysokosc2 -= 1;}
	if(aktualnaWysokosc2 < tabWysokosci[way2]){aktualnaWysokosc2 += 1;}
