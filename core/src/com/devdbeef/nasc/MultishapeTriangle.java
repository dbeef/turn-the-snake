...
public class MultishapeTriangle implements Screen {
...
public Rectangle[] rects0= new Rectangle[800];
...
public Rectangle[] rects4= new Rectangle[400];
  ...
  double timerSnake2 = 0;
	 ...
	 public class MyActor extends Actor {
	        ...
	        game.sps.setRegion(20, 140, 430, 493);
	        game.sps.setX(165);
	        game.sps.setY(-85);
	        ...
	        game.sps2.setRegion(20, 140, 430, 493);
	        game.sps2.setX(165);
	        game.sps2.setY(-302);
...
public MultishapeTriangle(final Starter myGdxGame) {
...
///
	...
	Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);	
		...
		//drawFigureRectangles();
		game.batch.setColor(1,1,1,(float)timerPentagon);
		game.batch.draw(game.yellowTriangle,0,10);
		drawSnake();
		drawApple();
		...
		game.batch.setProjectionMatrix(camera2.combined);
			...
			game.batch.end();
			game.fbo2.end();
//System.out.println(appleX1 + " applex1 + appley1 " + appleY1);
//System.out.println(appleX2 + " applex2 + appley2 " + appleY2);
//System.out.println(snakeHead2.x + " +y " + snakeHead2.y);
...
void spawnSnake(){
	...
	snakeX2 = (float) snakeX2 + waysX[way2]*(float)(ogniwa2 - 1f);
	snakeY2 = (float) snakeY2 + waysY[way2]*(float)(ogniwa2 - 1f);
...
//	kolizja = true;}
//	if(rects4[a].overlaps(snakeHead)){lost = true;if(kolizja == false)game.przegrana.play();
//	kolizja = true;}
	}
	if(way > 2)way = 0;
	if(way2 > 2)way2 = 0;
...
int random = MathUtils.random(1, 7);
	int x= MathUtils.random(120,300);
	int y= MathUtils.random(250,360);
...
public void goOut(){
	...
	if(tryAgainBoolean == true && timerOut == 0.9f){	game.setScreen(new MultishapeTriangle(game));
	}
